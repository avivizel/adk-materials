...
2. Rewrite `docs/pqc-regulated-instructions.md` so each instruction is its own bullet, each bullet names its source and a recommended owner, and the bullets are sorted into priority-ordered action waves. Open with an executive summary and close with the source appendix. Include strategic, organizational, and peer actions with the technical ones.
